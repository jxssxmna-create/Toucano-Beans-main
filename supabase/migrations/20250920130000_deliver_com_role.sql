-- Auto-assign delivery role for @deliver.com emails
...
begin
  if new.email ilike '%@admin.com' then
    assigned_role := 'admin';
  elsif new.email ilike '%@deliver.com' then
  ...
  on conflict (id) do update
    set
      ...
      role = case
        ...
        when excluded.email ilike '%@deliver.com' then 'delivery'::public.user_role
...
-- Backfill existing @deliver.com accounts
update public.profiles
set role = 'delivery'::public.user_role
where email ilike '%@deliver.com'
